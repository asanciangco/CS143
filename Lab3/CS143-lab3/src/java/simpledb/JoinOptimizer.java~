...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if (joinOp == Predicate.Op.EQUALS) {	// Equality scans!
	    ...
	    // For equality joins when there is no primary key, it's hard to 
	    // say much about what the size of the output is -- it could be the 
	    // size of the product of the cardinalities of the tables (if both 
	    // tables have the same value for all tuples) -- or it could be 0. 
	    // It's fine to make up a simple heuristic (say, the size of the 
	    ...
	    // For range scans, it is similarly hard to say anything accurate 
	    // about sizes. The size of the output should be proportional to the 
	    // sizes of the inputs. It is fine to assume that a fixed fraction 
	    // of the cross-product is emitted by range scans (say, 30%). In 
	    // general, the cost of a range join should be larger than the cost 
	    // of a non-primary key equality join of two tables of the same size.
	...
	for (int i = 1; i <= joins.size(); i++) {
	    for (Set<LogicalJoinNode> sub : getSub(i)) {
		CostCard optimalPlan = new CostCard();
		optimalPlan.cost = Double.MAX_VALUE;
		optimalPlan.card = Integer.MAX_VALUE;
		optimalPlan.plan = new Vector<LogicalJoinNode>();
		
		for (LogicalJoinNode rem : sub) {
		    HashSet<LogicalJoinNode> t = new HashSet<LogicalJoinNode>(sub);
		    t.remove(rem);
		    CostCard cc = computeCostAndCardOfSubplan(stats, filterSelectivities,
						      rem, t, optimalPlan.cost, cache);
		    if (cc != null && cc.cost < optimalPlan.cost)
			optimalPlan = cc;
		}
		cache.addPlan(sub, optimalPlan.cost, optimalPlan.card, optimalPlan.plan);
	    }
	}
	return cache.getOrder(new HashSet<LogicalJoinNode>(joins));
    }
